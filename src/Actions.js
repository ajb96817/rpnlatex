...
class InputContext {
    ...
    for(let i = 0; i < commands.length; i++) {
      ...
      try {
        ...
        if(['stack_underflow', 'stack_type_error',
          ...
          this.error_message = {
            message: this._reformat_error_message(e.message),
  ...
  _reformat_error_message(s) {
    // if(s === 'stack_underflow') return 'Stack underflow';
    // if(s === 'stack_type_error') return 'Wrong item type';
    // if(s === 'prefix_argument_required') return 'Prefix argument required';
    if(s.startsWith('Stop: '))
      s = s.slice(6);
    if(s.includes('tensor dimension check'))
      return 'Matrix sizes incompatible';
    return s;
  }
